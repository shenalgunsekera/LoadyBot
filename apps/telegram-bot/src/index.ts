...
import {
  money, parseAmount, resolvePlayer, platformsFor, methodsFor, adminChatFor,
  type Player,
} from './ui';
...
/**
 * The ONE shared Loady Telegram bot. A message is routed to its club by the chat
 * it arrived in (bound group) or, in a DM, by the player's join-token link. All
 * data access happens inside withAccount(), so RLS seals each club off.
 */
...
type Ctx = Context & SessionFlavor<SessionData> & { account?: Account; player?: Player };
...
// ── /start [token] — bind a DM to a club via its deep link, or greet ─────────
...
// ── /connect <CODE> — bind this chat to the account that generated the code ──
...
const player = (ctx: Ctx, account: Account) =>
...
// ── Text: amount / payout handle steps ───────────────────────────────────────
    ...
    try {
      const info = await withAccount(account.id, async (sql) => {
        ...
        const [f] = await sql<{ id: string; withdraw_id: string | null; payout_handle: string | null; club_handle: string | null }[]>`
          select f.id, f.withdraw_id, f.payout_handle, pm.club_handle
      ...
      await ctx.reply(
        `💸 *Send ${money(amount)} now.*\n\n` +
          (handle ? `Pay to: \`${handle}\`\n\n` : `An admin will send you where to pay shortly.\n\n`) +
          `Once you’ve paid, send a *screenshot* here so we can confirm it and add your money.`,
        { parse_mode: 'Markdown' },
      );
    } catch (e) {
...
// ── Receipt photo → submit proof + alert the admin chat with Verify ──────────
...
bot.callbackQuery(/^v:(.+)$/, async (ctx) => {
  const account = await accountForChat('telegram', String(ctx.chat!.id));
  if (!account || !(await isAccountAdmin(account.id, 'telegram', String(ctx.from.id)))) {
    return ctx.answerCallbackQuery({ text: 'Admins only.' });
  }
  try {
    ...
    await ctx.answerCallbackQuery({ text: errText(e).slice(0, 190) });
  }
});
